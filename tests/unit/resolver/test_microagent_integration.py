...
def test_resolver_uses_knowledge_microagents():
    ...
    resolver = GithubIssueHandler(owner='test', repo='test', token='test-token')
...
"""
    ...
    with tempfile.NamedTemporaryFile(suffix='.md') as f:
        ...
        with mock.patch(
            'openhands.microagent.registry.knowledge_agents', {agent.name: agent}
        ):
            ...
            assert not any(
                'flargle settings before bargling' in ctx for ctx in unrelated_contexts
            )
...
"""
    ...
    with tempfile.NamedTemporaryFile(suffix='.md') as f:
        ...
        with mock.patch(
            ...
            review_threads = [
                ReviewThread(
                    comment='This might affect performance', files=['widget.py']
                )
            ]
